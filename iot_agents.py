...
class QNetwork(nn.Module):
    ...
    def forward(self, state, action=None):
        if action is None:
            ...
            # For continuous actions, we concatenate state and action
...
class SAC:
    ...
    def __init__(self,action_space, state_space, gamma, alpha, hidden_size, lr=1e-3, use_cuda=False, target_update_interval=10):
        ...
        self.critic = QNetwork(state_space.shape[0], action_space.shape[0]).to(device=self.device)
...
class SafeSAC(SAC):
    def __init__(self, *args, gamma_safe=0.99, epsilon=0.2, safe_lr=1e-3, tau=0.005, **kwargs):
        ...
        self.qsafe = QNetwork(self.state_dim, self.action_dim).to(self.device)
        self.qsafe_target = QNetwork(self.state_dim, self.action_dim).to(self.device)
...
class DiscretePolicy(nn.Module):
    def __init__(self, state_dim, action_dim):
        super().__init__()
        self.fc1 = nn.Linear(state_dim, 512)  # Increased hidden layer size
        self.fc2 = nn.Linear(512, 256)
        self.fc3 = nn.Linear(256, 128)
...
class DiscreteSACAgent:
    ...
    def update(self, memory, batch_size):
        ...
        if isinstance(state[0], dict):
            state = np.array([flatten_state(s) for s in state])
            next_state = np.array([flatten_state(s) for s in next_state])
    ...
    def act(self, state, env=None, action_mask=None, eval=False):
        ...
        with torch.no_grad():
            if eval:
                # Use argmax for deterministic action selection during evaluation
                action_probs = self.policy(state)
                action_idx = torch.argmax(action_probs, dim=1).item()
            else:
                # Sample from the policy during training
                action_probs = self.policy(state)
            ...
            if env is None:
                raise ValueError("Environment must be provided to generate actions")
            ...
            n_ports = len(env.ports)

            # Calculate section sizes
            connect_actions = max_nodes * max_nodes * n_ports
            local_exploit_actions = max_nodes * n_local_vulns
            remote_exploit_actions = max_nodes * max_nodes * n_remote_vulns

            # Create action array with proper shape for CyberBattle
            action = np.zeros(5, dtype=np.int64)  # Maximum size needed for any action type

            # Determine action type based on which section the action_idx falls into
            if action_idx < connect_actions:
                # Connect action
                action[0] = 0
                remaining_idx = action_idx
                action[1] = remaining_idx % max_nodes  # source node
                remaining_idx //= max_nodes
                action[2] = remaining_idx % max_nodes  # target node
                action[3] = remaining_idx // max_nodes  # port index
            elif action_idx < connect_actions + local_exploit_actions:
                # Local exploit action
                action[0] = 1
                remaining_idx = action_idx - connect_actions
                action[1] = remaining_idx % max_nodes  # node index
                action[2] = remaining_idx // max_nodes  # vulnerability index
            else:
                # Remote exploit action
                action[0] = 2
                remaining_idx = action_idx - (connect_actions + local_exploit_actions)
                action[1] = remaining_idx % max_nodes  # source node
                remaining_idx //= max_nodes
                action[2] = remaining_idx % max_nodes  # target node
                action[3] = remaining_idx // max_nodes  # vulnerability index
